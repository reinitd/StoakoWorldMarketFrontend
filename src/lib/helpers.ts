export function deserialize<T>(data: any, classType: { new(...args: any[]): T }): T {
...
export function formatUnixTimeMMDDYYYY(unixTime: number): string {
...
export function getParamFromUrl<T>(paramName: string, paramType: { new(value: string): T }): T | null {
...
export async function makeCeHtml(ce: CatalogEntry) {
...
export async function makeVlCeHtml(ce: CatalogEntry) {
    const tr = document.createElement('tr');
